PLOVER_VERSION=4.0.3
PLOVER_APPIMAGE=plover-${PLOVER_VERSION}-x86_64.AppImage
if test ! -e ~/.local/bin/${PLOVER_APPIMAGE}
then
  echo "installing plover"
  rm ~/.local/bin/plover*
  wget https://github.com/openstenoproject/plover/releases/download/v${PLOVER_VERSION}/plover-${PLOVER_VERSION}-x86_64.AppImage -O ~/.local/bin/${PLOVER_APPIMAGE}
